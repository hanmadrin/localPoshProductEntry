...
const contentSetup = async()=>{
    ...
    if(isLocalPosh){
        ...
        contentScripts.showDebugButton('nextItem',async()=>{
            const res = await fetch(`http://127.0.0.1:5656/nextItem`);
            const data = await res.json();
            await db.SET(data);
            window.location.href = data.url;
        });
        contentScripts.showDebugButton('gotToCurrent',async()=>{
            const res = await fetch(`http://127.0.0.1:5656/nextItem`);
            const data = await res.json();
            console.log(data)
            // await db.SET(data);
        ...
        contentScripts.showDebugButton('currentDone',async()=>{
            const data = await db.GET();
            const key = data.key;
